...
require_once '../model/TraducaoIA.php';
...
try {
    ...
    if ($action === 'melhorar') {
        $frase = trim($input['phrase'] ?? '');
        $sourceLang = trim($input['sourceLang'] ?? '');
        $targetLang = trim($input['targetLang'] ?? '');

        if ($frase === '' || $sourceLang === '' || $targetLang === '') {
        ...
        $resultado = TraducaoIA::melhorarTraducao($pdo, $chat, $user_id, $plano, $frase, $sourceLang, $targetLang);
