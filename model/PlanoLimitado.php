...
// natural, Frase do Dia, Perguntas, Categoria com IA, Melhorar tradução).
// Quando o usuário esgota todas as amostras que efetivamente experimentou,
// não sobra mais nenhuma amostra grátis das ferramentas que ele usa - a
// partir daí ele é, na prática, um usuário free (plano 2), então o próprio
// sistema faz esse rebaixamento automático em vez de deixar o usuário
// "preso" num plano que não libera mais nada mas também não é tratado como
// free em nenhuma outra parte do app.
//
...
class PlanoLimitado
{
    ...
    const LIMITE_PERGUNTAS_VITALICIO = 3;
    ...
    public static function todasFerramentasTentadasEsgotadas(PDO $pdo, int $user_id): bool
    {
        $usos = [
            ...
            [self::contarComStatus($pdo, 'perguntas_ia', $user_id), self::LIMITE_PERGUNTAS_VITALICIO],
