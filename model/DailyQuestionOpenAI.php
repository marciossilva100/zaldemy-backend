...
class DailyQuestionOpenAI
{
    ...
    public static function obterPergunta(PDO $pdo, OpenAiChat $chat, int $user_id, array $phrases, string $idiomaNome, string $idiomaNativoNome, ?string $nivelNome = null, ?array $categoriaIds = null): array
    {
        ...
        $systemPrompt = "Você é um professor de idiomas. Crie UMA pergunta simples em {$idiomaNome}, pra um aluno de "
            ...
            . "só uma. Revise também o MODO VERBAL de cada oração, não só o tempo, de acordo com as regras "
            . "gramaticais PRÓPRIAS de {$idiomaNome} - erro comum em línguas que têm modo subjuntivo: usar "
            . "indicativo (futuro/presente) numa oração que a conjunção exige subjuntivo. Exemplo do erro em "
            . "português (só pra ilustrar o TIPO de erro, aplique a regra equivalente de {$idiomaNome} se ela "
            . "existir nesse idioma): depois de \"para que\", \"embora\", \"caso\", \"a menos que\", o verbo tem "
            . "que ir pro subjuntivo (\"para que você chegue lá\", NUNCA \"para que você chegará lá\"). Se "
            . "{$idiomaNome} não tiver esse conceito de modo subjuntivo, ignore esse exemplo e revise só as regras "
            . "de modo/tempo verbal que existem de fato nesse idioma. O campo \"pergunta\" tem que conter SÓ a "
    ...
    private static function avaliarESalvarResposta(
        ...
        $systemPrompt = "Você é um professor de idiomas avaliando a resposta {$tipoResposta} de um aluno. Vai receber a "
            ...
            . "\"incompleta\"/sem frase completa, avalie normalmente o conteúdo. IMPORTANTE: erro gramatical NÃO é a "
            . "mesma coisa que idioma errado - uma frase com concordância ruim, tempo verbal errado, artigo faltando "
            . "ou mal construída, mas formada 100% por palavras de {$idiomaNome}, continua no idioma CERTO (só com "
            . "nota baixa pela qualidade gramatical); só conte como idioma errado quando existir de fato uma palavra "
            . "de outro idioma no meio da frase. Nunca escreva no feedback que a resposta \"não está em {$idiomaNome}\" "
            . "ou \"contém palavras de outro idioma\" se isso não for literalmente verdade - descreva o problema real "
            . "(gramática, clareza, não responder à pergunta) em vez disso. Se estiver no idioma certo, avalie "
        ...
        // JÁ FOI CORTADO NO MEIO DA FRASE 2x por um orçamento de tokens
        // curto demais pra "feedback" + "resposta_ideal" no mesmo JSON (900
        // ainda não era suficiente numa resposta mais longa) - 1600 é folga
        // grande o bastante pra nunca faltar espaço pros dois campos em uso
        // normal (custo extra é zero, max_tokens é só um teto, não é cobrado
        // o que não é gerado).
        ...
        // 300 e depois 500 já cortaram no meio da explicação mesmo com a IA
        // respeitando o pedido de "máx 200/300 caracteres" no prompt - o
        // texto tá em português (mais verboso que o pedido em caracteres
        // sugere) e faz parte de um JSON com outro campo grande do lado.
        // 2000 é só uma trava de sanidade contra a IA enlouquecer (repetição
        // infinita etc) - não deve cortar NADA em uso normal, nunca mais.
        $feedback = self::truncarPreservandoPalavras((string) ($correcao['feedback'] ?? ''), 2000);
        $respostaIdeal = self::truncarPreservandoPalavras((string) ($correcao['resposta_ideal'] ?? ''), 2000);
